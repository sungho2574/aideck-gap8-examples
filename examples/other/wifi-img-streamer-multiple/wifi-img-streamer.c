...
#ifndef HOTSPOT_SSID
#error "HOTSPOT_SSID must be defined at build time: make HOTSPOT_SSID=MyWifi HOTSPOT_PASSWORD=mypass"
#endif
#ifndef HOTSPOT_PASSWORD
#error "HOTSPOT_PASSWORD must be defined at build time: make HOTSPOT_SSID=MyWifi HOTSPOT_PASSWORD=mypass"
#endif
...
void setupWiFi(void) {
  static char ssid[] = HOTSPOT_SSID;
  static char key[]  = HOTSPOT_PASSWORD;
  cpxPrintToConsole(LOG_TO_CRTP, "Connecting to WiFi STA: %s\n", ssid);
  ...
  // Connect as STA (station) mode: 0x00 = STA, 0x01 = AP
